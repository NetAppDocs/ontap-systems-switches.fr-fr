...
* Déclenchez une maintenance AutoSupport avant et après celle-ci afin de désactiver la création de tickets pendant toute sa durée. Consultez cet article de la base de connaissances https://kb.netapp.com/Support_Bulletins/Customer_Bulletins/SU92["SU92 : Comment désactiver la création automatique de tickets pendant les fenêtres de maintenance planifiées ?"^] pour plus d'informations.
...
----
====
. Activez tous les ports, Eth1/1 à Eth1/30, sur le commutateur newcs2.
+
.Afficher un exemple
[%collapsible]
====
...
. Vérifiez la connectivité des interfaces du cluster distant :
+
.. Vous pouvez utiliser le `network interface check cluster-connectivity` commande permettant d'afficher les détails d'un contrôle d'accessibilité pour la connectivité du cluster :
+
[source, cli]
----
