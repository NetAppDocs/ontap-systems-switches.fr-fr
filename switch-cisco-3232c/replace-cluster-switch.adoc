...
Les exemples de cette procédure de remplacement utilisent quatre nœuds. Deux des nœuds utilisent quatre ports d'interconnexion de cluster de 10 GB : e0a, e0b, e0c et e0d. Les deux autres nœuds utilisent deux ports d'interconnexion de cluster de 40 GB : e4a et e4e. Reportez-vous à la link:https://hwu.netapp.com["Hardware Universe"^] pour vérifier les ports de cluster corrects pour votre plateforme.
