...
[role="lead"]
Le système d'exploitation Cumulus Linux (CL) peut être installé lorsque le commutateur exécute Cumulus Linux ou ONIE.

Les hypothèses suivantes sont faites :

* Vous disposez de connaissances Linux de niveau intermédiaire.
* Vous connaissez bien l'édition de texte de base, les autorisations de fichiers UNIX et la surveillance des processus. Plusieurs éditeurs de texte sont pré-installés, y compris `vi` et `nano`.
* Vous devez avoir accès à un shell Linux ou UNIX. Si vous utilisez Windows, utilisez un environnement Linux comme outil de ligne de commande pour interagir avec Cumulus Linux.
...
.Étapes
. Connectez-vous au commutateur. La première connexion au commutateur requiert le nom d'utilisateur/mot de passe *cumulus*/*cumulus* avec `sudo` privilèges :
+
[listing, subs="+quotes"]
----
