...
[role="lead"]
Suivre cette procédure pour installer la FCR après la première configuration du commutateur Nexus 92300YC. Vous pouvez également utiliser cette procédure pour mettre à niveau votre version RCF.
...
----
====
.. Vérifier que toutes les interfaces de cluster (LIFs) se trouvent sur le port de home port : +
...
----
====
.. Vérifier que le cluster affiche les informations relatives aux deux commutateurs du cluster : +
...
.Et la suite ?
link:setup-install-cshm-file.html["Installez le fichier de configuration CSHM (Cluster Switch Health Monitor)"].
