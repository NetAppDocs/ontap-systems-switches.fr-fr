...
---
= Installez ou mettez à niveau le script du fichier de configuration de référence (RCF).
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Cumulus Linux est installé. Consultez la link:https://hwu.netapp.com/Switch/Index["Hardware Universe"^] pour connaître les versions prises en charge.
...
| Cluster | Sur les ports configurés pour une interface 4x10GbE, un port est configuré pour une interface 4x25GbE et les autres ports sont configurés pour une interface 40/100GbE. Prend en charge le trafic partagé de cluster/HA sur les ports pour les nœuds qui utilisent des ports partagés de cluster/HA. Consultez le tableau des plateformes dans l’article de la Knowledge Base link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/What_AFF_ASA_and_FAS_platforms_use_shared_Cluster_and_HA_Ethernet_ports["Quelles plates-formes AFF, ASA et FAS utilisent des ports Ethernet Cluster et HA partagés ?"^]. Tous les ports peuvent également être utilisés comme ports de cluster dédiés. 
...
. Utilisez le `ping` commande permettant de vérifier la connectivité au serveur hébergeant Cumulus Linux et le RCF.
...
** Si vous venez de mettre à jour votre version de Cumulus Linux, vous n'avez pas besoin de désactiver la restauration automatique pour cette étape car elle est déjà désactivée.
...
NOTE: Alors que `scp` Si le protocole utilisé dans cet exemple est utilisé, vous pouvez utiliser votre méthode de transfert de fichiers préférée, par exemple SFTP, HTTPS ou FTP.
...
. Répétez les étapes 1 à 14 sur le deuxième interrupteur.
...
NOTE: À l'étape 3 *Mise à jour du fichier MOTD* ci-dessus, la commande `cat /etc/issue.net` est exécuté.  Cela vous permet de vérifier le nom du fichier RCF, la version RCF, les ports à utiliser et d'autres informations importantes dans la bannière RCF.
...
NOTE: Bien que `scp` Si le protocole utilisé dans cet exemple est utilisé, vous pouvez utiliser votre méthode de transfert de fichiers préférée, par exemple SFTP, HTTPS ou FTP.
...
NOTE: À l'étape 3 *Mise à jour du fichier MOTD* ci-dessus, la commande `cat /etc/issue.net` est exécutée.  Cela vous permet de vérifier le nom du fichier RCF, la version RCF, les ports à utiliser et d'autres informations importantes dans la bannière RCF.
...
Une fois le RCF installé, vous pouvez link:setup-install-cshm-file.html["installer le fichier CSHM"].
