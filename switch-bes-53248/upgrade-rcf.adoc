...
summary: Vous pouvez mettre à niveau le RCF après avoir configuré le commutateur de cluster BES-53248 et après avoir appliqué de nouvelles licences. 
---
= Mettre à niveau la version RCF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Vous pouvez mettre à niveau le RCF après avoir mis à niveau l'EFOS du commutateur de cluster BES-53248 et après avoir appliqué toute nouvelle licence.
...
* Consultez le tableau de compatibilité des commutateurs pour connaître les versions ONTAP et RCF prises en charge. Reportez-vous à link:https://mysupport.netapp.com/site/info/broadcom-cluster-switch["Téléchargement du logiciel EFOS"^]. Notez qu'il peut exister des dépendances entre la syntaxe des commandes dans le RCF et celle trouvée dans les versions d'EFOS.
* Consultez les guides logiciels et de mise à niveau appropriés disponibles sur le link:https://www.broadcom.com/support/bes-switch["Broadcom"^] site pour une documentation complète sur les procédures de mise à niveau et de rétrogradation du commutateur BES-53248.
...
* Les exemples de cette procédure utilisent quatre nœuds. Ces nœuds utilisent deux ports d'interconnexion de cluster 10GbE `e0a` et `e0b`. Consultez la link:https://hwu.netapp.com/Home/Index["Hardware Universe"^] pour vérifier les ports de cluster appropriés sur vos plateformes.
...
show isdp neighbor
----
. Sur le commutateur cs2, désactivez les ports connectés aux ports de cluster des nœuds.  Par exemple, si les ports 0/1 à 0/16 sont connectés à des nœuds ONTAP :
+
[listing, subs="+quotes"]
----
(cs2)> *enable*
...
. Notez ce qui suit :
+
.. Si des licences de port supplémentaires ont été installées sur le commutateur, vous devez modifier le RCF pour configurer les ports supplémentaires sous licence. Reportez-vous à link:configure-licenses.html#activate-newly-licensed-ports["Activer les ports nouvellement autorisés"] pour plus de détails. Cependant, lorsque vous effectuez une mise à niveau vers RCF 1.12 ou une version ultérieure, ces modifications ne sont plus nécessaires car toutes les interfaces sont désormais préconfigurées.
...
CAUTION: Le fichier doit être nommé  `reference-config.scr` lorsque vous chargez et appliquez le fichier RCF, sinon l'étape d'application du script échouera.

. Copiez le RCF sur le commutateur cs2 en utilisant l'un des protocoles de transfert suivants : HTTP, HTTPS, FTP, SFTP ou SCP.
+
...
--
====
. [[étape 11]]Examinez la bannière affichée par le `show clibanner` commande. Vous devez lire et suivre ces instructions pour garantir la configuration et le fonctionnement corrects du commutateur.
+
`show clibanner`
...
----
. Sur le commutateur de cluster cs2, activez les ports connectés aux ports de cluster des nœuds.
+
[listing, subs="+quotes"]
----
(cs2)> *enable*
...
--
====
. [[étape 21]]Répétez les étapes 1 à 20 sur le commutateur cs1.
...
----
+
Pour plus de détails, veuillez vous référer à link:https://docs.netapp.com/us-en/ontap/networking/revert_a_lif_to_its_home_port.html["Rétablir le port d'origine d'une interface réseau locale (LIF)"].
