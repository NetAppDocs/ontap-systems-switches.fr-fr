...
---
= Comment remplacer les commutateurs de cluster Cisco Nexus 3132Q-V.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Le remplacement d'un commutateur Cisco Nexus 3132Q-V défectueux sur un réseau de clusters est une procédure de CONTINUITÉ de L'activité. Vous devez alors effectuer une séquence de tâches spécifique.

.Avant de commencer
* Le cluster et la configuration réseau existants doivent présenter les éléments suivants :
+
** L'infrastructure de cluster Nexus 3132Q-V doit être redondante et entièrement fonctionnelle sur les deux commutateurs.
+
...
** Toutes les interfaces logiques de cluster (LIF) doivent se trouver dans le `up` état et ne doit pas avoir été migré.


* Le commutateur de remplacement Nexus 3132Q-V :
+
...
** Le commutateur d'image RCF souhaité et du système d'exploitation NX-OS doit être chargé sur le commutateur.
...
Cette procédure remplace le deuxième commutateur CL2 du bloc de commutateurs Nexus 3132Q-V par le nouveau commutateur 332Q-V C2. Les exemples de cette procédure utilisent la nomenclature des commutateurs et des nœuds suivante :
...
* Le commutateur CL2 du bloc d'instruments doit être remplacé par C2 (<<step1_replace3132q,Étapes 1>> -<<step21_replace3132q,21>>)
+
...
.Étapes
. [[step1_replace3132q]]si AutoSupport est activé sur ce cluster, supprimez la création automatique de dossiers en invoquant un message AutoSupport :
+
...
1     Po1(SU)      Eth    LACP      Eth1/31(P)   Eth1/32(P)
----
. Sur tous les nœuds, ajouter tous les ports d'interconnexion de cluster connectés au commutateur Nexus 3132Q-V C2 : `network port modify`
+
[listing]
----
...
----
. [[step21_replace3132q]]retirez le commutateur Nexus 3132Q-V remplacé, s'il n'est pas déjà supprimé automatiquement :
+
