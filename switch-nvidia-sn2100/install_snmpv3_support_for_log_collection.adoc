...
summary: Cette version inclut la prise en charge du protocole SNMPv3 pour la surveillance des journaux. 
---
= Configurer SNMPv3 pour la collecte log switch
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Cette version inclut la prise en charge du protocole SNMPv3 pour la collecte du journal de commutation pour Switch Health Monitoring (SHM).
...
----
. Configurez SHM pour qu'il surveille avec le nouvel utilisateur SNMPv3 :
+
[listing, subs="+quotes"]
----
...
----
. Vérifiez que le numéro de série à interroger avec l'utilisateur SNMPv3 nouvellement créé est identique à celui détaillé à l'étape précédente une fois la période d'interrogation SHM terminée.
+
[listing, subs="+quotes"]
----
