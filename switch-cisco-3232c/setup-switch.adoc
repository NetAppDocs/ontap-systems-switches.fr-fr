...
* NetApp applicable au réseau de clusters et au réseau de gestion, téléchargés depuis le site de support NetApp à l'adresse http://["mysupport.netapp.com"^] pour les commutateurs que vous recevez. Tous les commutateurs de réseau de gestion et de réseau de cluster Cisco sont livrés avec la configuration par défaut standard de Cisco. Ces commutateurs ont également la version actuelle du logiciel NX-OS, mais ils ne sont pas chargés.
