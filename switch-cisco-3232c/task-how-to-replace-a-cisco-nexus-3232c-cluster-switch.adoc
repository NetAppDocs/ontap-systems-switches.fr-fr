...
---
= Comment remplacer un switch de cluster Cisco Nexus 3232C
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Vous pouvez remplacer un switch Cisco Nexus 3232C défectueux dans un cluster sans interruption en exécutant une séquence de tâches spécifique.

.Avant de commencer
Les caractéristiques suivantes de la configuration existante du cluster et du réseau sont les suivantes :

* L'infrastructure de cluster Nexus 3232C doit être redondante et entièrement fonctionnelle sur les deux switchs.
+
...
* Toutes les interfaces logiques de cluster (LIF) doivent être à l'état up et ne doivent pas avoir été migrées.
...
* *Afficher et migrer les ports du cluster vers le commutateur C2 (étapes <<one,1>>-<<seven,7)>>*
* *Rebrancher les câbles ISL du commutateur CL2 au commutateur C2, puis migrer les liens ISL vers le commutateur CL1 et C2 (étapes <<eight,8>>-<<fourteen,14)>>*
* *Revert toutes les LIFs vers les ports attribués à l'origine (étapes <<fifteen,15>>-<<eighteen,18)>>*
* *Vérifiez que tous les ports et LIF sont correctement migrés (étapes <<nineteen,19>>-<<twentyone,21>>)*


Cette procédure remplace le second switch de cluster Nexus 3232C CL2 par le nouveau switch 3232C de C2. Les exemples de cette procédure utilisent la nomenclature des commutateurs et des nœuds suivante :
...
Les exemples de cette procédure utilisent quatre nœuds. Deux des nœuds utilisent quatre ports d'interconnexion de cluster de 10 Go : e0a, e0b, e0c et e0d. Les deux autres nœuds utilisent deux ports d'interconnexion de cluster de 40 Go : e4a et e4e. Voir la link:https://hwu.netapp.com/SWITCH/INDEX.html["Hardware Universe"^] pour vérifier les ports de cluster appropriés pour votre plate-forme.

Cette procédure décrit le scénario suivant :
...
+ _x_ est la durée de la fenêtre de maintenance en heures.
...
+
https://mysupport.netapp.com/NOW/download/software/cm_switches/["Commutateurs Ethernet Cisco"^]
...
12 entries were displayed.
----
. Arrêtez les ports d'interconnexion de cluster qui sont physiquement connectés au commutateur d'origine CL2 : `network port modify -node _node-name_ -port _port-name_ -up-admin false`
+
...
----
. [[sept]]Envoyez une commande ping aux interfaces de cluster distantes et effectuez une vérification de serveur RPC :
+
...
----
. [[huit]]arrêtez les ports 1/31 et 1/32 du commutateur de cluster CL1.
+
...
1     Po1(SU)      Eth    LACP      Eth1/31(P)   Eth1/32(P)
----
. [[14]]sur tous les nœuds, activer tous les ports d'interconnexion de cluster connectés au commutateur de remplacement C2 : `network port modify -node _node-name_ -port _port-name_ -up-admin true`
+
[listing]
----
...
----
. [[quinze]]Retour à toutes les LIFs d'interconnexion de cluster migrées sur tous les nœuds :
+
...
e4e       Cluster      Cluster          up   9000 auto/40000  -
        12 entries were displayed.
----
. [[dix-huit]]Envoyez une commande ping aux interfaces de cluster distantes et effectuez un contrôle du serveur RPC :
+
`cluster ping-cluster -node _node-name_`
...
8	paths up, 0 paths down (udp check)
----
. [[19]]pour afficher les informations sur les périphériques de votre configuration, entrez les commandes suivantes :
+
...
. [[twentyone]]Vérifiez que les commutateurs de cluster appropriés sont surveillés : `system cluster-switch show`
+
