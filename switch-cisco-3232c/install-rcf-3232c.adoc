...
Vous pouvez également utiliser cette procédure pour mettre à niveau votre version de RCF. Consultez l'article de la Knowledge Base link:https://kb.netapp.com/onprem/Switches/Cisco/How_to_clear_configuration_on_a_Cisco_interconnect_switch_while_retaining_remote_connectivity["Comment effacer la configuration d'un commutateur d'interconnexion Cisco tout en conservant la connectivité à distance"^] pour plus d'informations lors de la mise à niveau de votre RCF.
