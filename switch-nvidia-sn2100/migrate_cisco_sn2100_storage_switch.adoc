...
[role="lead"]
Vous devez connaître certaines informations de configuration, les connexions de ports et les exigences de câblage lorsque vous remplacez d'anciens switchs Cisco par des switchs de stockage NVIDIA SN2100.
...
Vous pouvez migrer d'anciens commutateurs de stockage Cisco sans interruption pour un cluster ONTAP vers des commutateurs de stockage NVIDIA SN2100.

* Le cluster existant doit être correctement configuré et opérationnel.
* Tous les ports de stockage doivent être à l'état « up » pour assurer la continuité de l'activité.
* Les commutateurs de stockage NVIDIA SN2100 doivent être configurés et fonctionnent sous la version appropriée de Cumulus Linux installée dans le fichier RCF (Reference Configuration File) appliqué.
* La configuration existante du réseau de stockage doit avoir les éléments suivants :
+
...
* Voir la https://hwu.netapp.com/["Hardware Universe"^] pour obtenir des détails complets sur les ports pris en charge et leurs configurations.
* Vous avez configuré certains des ports des switchs NVIDIA SN2100 pour qu'ils s'exécutent à 100 GbE.
* Vous avez planifié, migré et documenté la connectivité 100 GbE des nœuds vers les commutateurs de stockage NVIDIA SN2100.
+

NOTE: Dans cette procédure, les commutateurs de stockage Cisco Nexus 9336C-FX2 sont utilisés pour des commandes et des sorties par exemple.
...
.Étapes
. Si AutoSupport est activé, supprimez la création automatique de dossier en invoquant un message AutoSupport : `system node autosupport invoke -node * -type all -message MAINT=xh`
+
...
+
Afficher les attributs des ports réseau : `storage port show`
...
cluster1::*>
----
. Les ports de stockage de chaque nœud sont connectés aux commutateurs de stockage existants de la manière suivante (du point de vue des nœuds) à l'aide de la commande : `network device-discovery show -protocol lldp`
+
[listing, subs="+quotes"]
----
...
----
. Sur les commutateurs S1 et S2, les ports et les commutateurs de stockage sont connectés de la manière suivante (du point de vue des commutateurs) à l'aide de la commande : `show lldp neighbors`
+
[listing, subs="+quotes"]
----
...
cumulus@sw2:~$ *net commit*
----
. Les ports de stockage de chaque nœud sont désormais connectés aux switchs de la façon suivante, du point de vue des nœuds :
+
[listing, subs="+quotes"]
----
cluster1::*> *network device-discovery show -protocol lldp*
...
----
. Vérifiez les attributs des ports réseau : `storage port show`
+
[listing, subs="+quotes"]
----
cluster1::*> *storage port show*
...
node2      /lldp
            ...
            e5b    sw2  (b8:ce:f6:19:1a:7e)  swp2            -
----
. Vérifier la configuration finale : `storage port show`
+
