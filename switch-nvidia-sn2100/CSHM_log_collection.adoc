...
.Avant de commencer
* L'utilisateur de la collecte des journaux doit être spécifié lorsque le fichier RCF (Reference Configuration File) est appliqué. Par défaut, cet utilisateur est défini sur « admin ». Si vous souhaitez utiliser un autre utilisateur, vous devez le spécifier dans la section *# SHM User*s de la FCR.
