...
[role="lead"]
Vous pouvez installer le fichier de configuration de référence (RCF) après avoir configuré pour la première fois le commutateur Nexus 9336C-FX2.
...
* Les connexions ISL (Inter-Switch Link) entre les commutateurs sont connectées.
...
.Et la suite ?
Une fois le RCF installé, vous pouvez link:../switch-cshm/config-overview.html["Configurer la surveillance de l'état des commutateurs"].
