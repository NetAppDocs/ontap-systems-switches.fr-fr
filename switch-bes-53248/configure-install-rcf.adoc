...
[role="lead"]
Vous pouvez installer le RCF après avoir configuré le commutateur de cluster BES-53248 et après avoir appliqué les nouvelles licences.


CAUTION: Pour EFOS 3.12 et versions ultérieures, suivez les étapes d'installation dans link:efos-install-rcf-license-file.html["Installez le fichier de configuration de référence (RCF) et le fichier de licence"].
...
* Le fichier RCF actuel, disponible sur la page link:https://mysupport.netapp.com/site/products/all/details/broadcom-cluster-switches/downloads-tab["Commutateurs de cluster Broadcom"^].
...
. Notez ce qui suit :
+
.. Si des licences de port supplémentaires ont été installées sur le commutateur, vous devez modifier le RCF pour configurer les ports supplémentaires sous licence. Consultez link:configure-licenses.html#activate-newly-licensed-ports["Activer les ports nouvellement autorisés"] pour plus de détails.
