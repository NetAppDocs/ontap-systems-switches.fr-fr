...
---
= Installez le fichier de configuration de référence (RCF) et le fichier de licence
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
À partir d'EFOS 3.12.0.1, vous pouvez installer le RCF et le fichier de licence après avoir configuré le commutateur de cluster BES-53248.


CAUTION: Tous les ports sont configurés lors de l'installation de RCF, mais vous devez installer votre licence pour activer les ports configurés.
...
* Le RCF actuel, disponible sur la link:https://mysupport.netapp.com/site/products/all/details/broadcom-cluster-switches/downloads-tab["Commutateurs de cluster Broadcom"^] page.
...
* Une connexion console au commutateur, requise lors de l'installation du RCF à partir de l'état par défaut d'usine. Cette exigence est facultative si vous avez utilisé l'article de la Knowledge Base link:https://kb.netapp.com/onprem/Switches/Broadcom/How_to_clear_configuration_on_a_Broadcom_interconnect_switch_while_retaining_remote_connectivity["Comment effacer la configuration d'un commutateur d'interconnexion Broadcom tout en conservant la connectivité à distance"^] pour effacer la configuration au préalable.
...
CAUTION: Avant d'installer une nouvelle version du logiciel du commutateur et des RCF, utilisez l'article de la Knowledge Base link:https://kb.netapp.com/onprem/Switches/Broadcom/How_to_clear_configuration_on_a_Broadcom_interconnect_switch_while_retaining_remote_connectivity["Comment effacer la configuration d'un commutateur d'interconnexion Broadcom tout en conservant la connectivité à distance"^]. Si vous devez effacer complètement les paramètres du commutateur, vous devez effectuer à nouveau la configuration de base. Vous devez être connecté au commutateur à l'aide de la console série, car une suppression complète de la configuration réinitialise la configuration du réseau de gestion.
...
. Copiez le RCF dans la mémoire flash de démarrage du commutateur cs2 en utilisant l'un des protocoles de transfert suivants : FTP, HTTP, TFTP, SFTP ou SCP.
+
Cet exemple montre comment HTTP est utilisé pour copier un RCF dans la mémoire flash de démarrage du commutateur cs2 :
...
----
====
. Sur le commutateur, vérifiez que vos modifications ont bien été prises en compte :
+
[source, cli]
----
show running-config
...
--
====
. [[étape 21]]Sur le commutateur de cluster cs1, fermez les ports connectés aux ports de cluster des nœuds.
+
...
Une fois le fichier RCF et le fichier de licence installés, vous pouvez link:configure-ssh.html["activer SSH"].
