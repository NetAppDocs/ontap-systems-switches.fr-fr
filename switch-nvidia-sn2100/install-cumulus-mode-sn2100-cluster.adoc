...
NOTE: Le système d'exploitation Cumulus Linux (CL) peut être installé soit lorsque le commutateur exécute Cumulus Linux, soit lorsqu'il exécute ONIE (voirlink:install-onie-mode-sn2100-cluster.html["Installer en mode ONIE"] ).
...
+
La première connexion au commutateur nécessite un nom d'utilisateur et un mot de passe de *cumulus*/*cumulus* avec `sudo` privilèges.
...
----
+
Cette commande modifie à la fois le `/etc/hostname` et `/etc/hosts` fichiers.
...
----
.. Vérifiez que `ntpd` est en cours d'exécution sur le système :
+
[listing, subs="+quotes"]
----
...
. Vérifiez que la version de Cumulus Linux est bien la 4.4.3 : `net show version`
+
[listing, subs="+quotes"]
----
...
----
+
Consultez l'article de la base de connaissanceslink:https://kb.netapp.com/on-prem/Switches/Nvidia-KBs/NTP_Server_configuration_is_not_working_with_NVIDIA_SN2100_Switches["La configuration du serveur NTP ne fonctionne pas avec les commutateurs NVIDIA SN2100."^] pour plus de détails.
...
. Répétez les étapes 1 à 4 pour vous connecter.
. Vérifiez que la version de Cumulus Linux est bien la 5.4.0 : `nv show system`
+
[listing, subs="+quotes"]
----
cumulus@cumulus:mgmt:~$ *nv show system*
...
. Connectez-vous au commutateur.
+
Lors de votre première connexion au commutateur, le nom d'utilisateur et le mot de passe *cumulus*/*cumulus* vous seront demandés. `sudo` privilèges.
...
Voir https://docs.nvidia.com/networking-ethernet-software/cumulus-linux-511/System-Configuration/Authentication-Authorization-and-Accounting/User-Accounts/["Comptes utilisateurs NVIDIA"^] pour plus d'informations.

--
====
.Quelle est la prochaine étape ?
Après avoir installé Cumulus Linux en mode Cumulus, vous link:install-rcf-sn2100-cluster.html["installer le script du fichier de configuration de référence (RCF)"].
