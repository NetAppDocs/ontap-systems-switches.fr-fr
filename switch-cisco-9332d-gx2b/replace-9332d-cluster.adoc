...
summary: 'Le remplacement d"un commutateur Nexus 9332D-GX2B défectueux dans un réseau en cluster est une procédure non perturbatrice (NDU).' 
---
= Remplacer un commutateur Cisco Nexus 9332D-GX2B
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Suivez ces étapes pour remplacer un commutateur Nexus 9332D-GX2B défectueux dans un réseau partagé. Il s’agit d’une procédure non perturbatrice (NDU).
...
** L' ONTAP `cluster ping-cluster -node <node-name>` la commande doit indiquer que la connectivité de base et la communication supérieure à PMTU réussissent sur tous les chemins.


* Sur le commutateur de remplacement Nexus 9332D-GX2B :
+
...
** Les connexions de nœuds sont les ports 1/1 à 1/30.
** Tous les ports Inter-Switch Link (ISL) sont désactivés sur les ports 1/31 et 1/32.
** Le fichier RCF souhaité et le commutateur d'image du système d'exploitation NX-OS sont chargés sur ce commutateur.
** La personnalisation initiale du commutateur est terminée, comme détaillé danslink:setup-switch-9332d-cluster.html["Configurer le commutateur de cluster 9332D-GX2B"] .
+
...
* Les noms des commutateurs Nexus 9332D-GX2B existants sont cs1 et cs2.
* Le nom du nouveau commutateur Nexus 9332D-GX2B est newcs2.
...
. Sur le nouveau commutateur, connectez-vous en tant qu’administrateur et fermez tous les ports qui seront connectés aux interfaces du cluster de nœuds (ports 1/1 à 1/30).
+
...
----
====
.. Alternativement, vous pouvez également utiliser le `cluster ping-cluster -node <node-name>` commande pour vérifier la connectivité :
+
...
. Fermez les ports ISL Eth1/31 et Eth1/32 sur le commutateur Nexus 9332D-GX2B cs1.
+
[listing, subs="+quotes"]
----
cs1# *config*
...
cs1(config)# exit
----
. Retirez tous les câbles du commutateur Nexus 9332D-GX2B cs2, puis connectez-les aux mêmes ports sur le commutateur 9332D-GX2B newcs2.
. Activez les ports ISL Eth1/31 et Eth1/32 entre les commutateurs cs1 et newcs2, puis vérifiez l'état de fonctionnement du canal de port.
+
Port-Channel doit indiquer Po1(SU) et les ports membres doivent indiquer Eth1/31(P) et Eth1/32(P).

+
.Montrer l'exemple
[%collapsible]
====
Cet exemple active les ports ISL Eth1/31 et Eth1/32 et affiche le résumé du canal de port sur le commutateur cs1 :
...
Les commandes suivantes renvoient LIF `node1-01_clus2` sur `node1-01` au port d'attache `e7a` et affiche des informations sur les LIF sur les deux nœuds.  L'activation du premier nœud est réussie si la colonne Is Home est vraie pour les deux interfaces du cluster et qu'elles affichent les affectations de port correctes, dans cet exemple `e7a` et `e7b` sur le nœud 1-01.
...
. Vérifiez la connectivité des interfaces de cluster distantes :
+
.. Vous pouvez utiliser le `network interface check cluster-connectivity` commande pour afficher les détails d'une vérification d'accessibilité pour la connectivité du cluster :
+
[source, cli]
----
...
.Et la suite ?
Après avoir remplacé vos commutateurs, vouslink:../switch-cshm/config-overview.html["configurer la surveillance de l'état du commutateur"] .
