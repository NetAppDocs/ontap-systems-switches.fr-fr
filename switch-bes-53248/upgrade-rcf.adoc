...
summary: Vous pouvez mettre à niveau le fichier RCF après avoir configuré le commutateur de cluster BES-53248 et après avoir appliqué de nouvelles licences. 
---
= Mise à niveau du fichier RCF (Reference Configuration File)
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Vous pouvez mettre à niveau le fichier RCF (Reference Configuration File) après la mise à niveau du commutateur de cluster BES-53248 EFOS et après l'application de nouvelles licences.
...
show isdp neighbor
----
. Sur le commutateur cs2, arrêtez les ports connectés aux ports de cluster des nœuds. Par exemple, si les ports 0/1 à 0/16 sont connectés aux nœuds ONTAP :
+
[listing, subs="+quotes"]
----
(cs2)> *enable*
...
[role="tabbed-block"]
====
.EFOS version 3.12.x et ultérieure
--
...
--
.Toutes les autres versions de EFOS
--
. Copiez le RCF sur le bootflash du commutateur cs2 à l'aide de l'un des protocoles de transfert suivants : FTP, TFTP, SFTP ou SCP.
+
Cet exemple montre que SFTP est utilisé pour copier une FCR sur le bootflash du commutateur cs2 :
...
--
====
. [[step11]]Examinez la sortie de la bannière à partir de la `show clibanner` commande. Vous devez lire et suivre ces instructions pour vous assurer que la configuration et le fonctionnement du commutateur sont corrects.
+
`show clibanner`
...
--
====
. [[step21]]Répétez les étapes 1 à 20 sur le commutateur cs1.
