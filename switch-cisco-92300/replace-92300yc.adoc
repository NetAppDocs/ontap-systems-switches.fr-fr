...
summary: 'Le remplacement d"un commutateur Nexus 92300YC défectueux dans un réseau en cluster est une procédure non perturbatrice (NDU).' 
---
= Remplacer un commutateur Cisco Nexus 92300YC
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Le remplacement d'un commutateur Nexus 92300YC défectueux dans un réseau en cluster est une procédure non perturbatrice (NDU).
...
** Les connexions de nœuds sont les ports 1/1 à 1/64.
** Tous les ports Inter-Switch Link (ISL) sont désactivés sur les ports 1/65 et 1/66.
** Le fichier de configuration de référence souhaité (RCF) et l'image du système d'exploitation NX-OS sont chargés sur le commutateur.
** La personnalisation initiale du commutateur est terminée, comme détaillé dans :link:configure-install-initial.html["Configurer le commutateur Cisco Nexus 92300YC"] .
+
...
. Désactivez les ports ISL 1/65 et 1/66 sur le commutateur Nexus 92300YC cs1 :
+
.Afficher un exemple
[%collapsible]
====
...
----
====
. Retirez tous les câbles du commutateur Nexus 92300YC cs2, puis connectez-les aux mêmes ports sur le commutateur Nexus 92300YC newcs2.
. Activez les ports ISL 1/65 et 1/66 entre les commutateurs cs1 et newcs2, puis vérifiez l'état de fonctionnement du canal de port.
+
Port-Channel doit indiquer Po1(SU) et les ports membres doivent indiquer Eth1/65(P) et Eth1/66(P).
...
--
====
. [[étape 2]] Confirmez la configuration réseau du cluster suivante :
+
`network port show`
